...
async function saveConfig(user,rows){if(user.role!=='Administrador')throw new Error('Solo el Administrador puede modificar la configuración.');const payload=rows.map(x=>({Id:x.id||undefined,Grupo:x.group,Maquina:x.name,Componente:x.component,LimiteNormal:Number(x.normal),LimiteAlerta:Number(x.alert),LimitePeligro:Number(x.danger),ImagenRequerida:!!x.imageRequired,Activo:true,updated_at:new Date().toISOString()}));await upsert('tbTermoConfiguracion',payload,'Maquina,Componente');await writeAudit(user,'Configuración térmica actualizada',null,null,{registros:rows.length});return {saved:rows.length}}
