...
async function bootstrap(){
  const [cfg,paletteRows,rawIns,auditRows,imageRows,userRows]=await Promise.all([select('tbTermoConfiguracion','Activo=eq.true&order=Grupo.asc,Maquina.asc'),select('tbTermoPaletas','Activo=eq.true&order=Paleta.asc'),select('tbTermoInspecciones','order=Fecha.desc,created_at.desc'),select('tbTermoAuditoria','order=created_at.desc&limit=500'),select('tbTermoImagenes','Activo=eq.true'),select('tbUsuarios','order=Nombre.asc')]);
...
async function savePalettes(user,rows){if(user.role!=='Administrador')throw new Error('Solo el Administrador puede calibrar paletas.');const payload=rows.map(x=>({Paleta:x.name,TemperaturaMinima:Number(x.min),TemperaturaMaxima:Number(x.max),Emisividad:Number(x.emissivity),Distancia:Number(x.distance),Activo:true,updated_at:new Date().toISOString()}));await upsert('tbTermoPaletas',payload,'Paleta');await writeAudit(user,'Paletas calibradas',null,null,{registros:rows.length});return {saved:rows.length}}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(action==='savePalettes')return res.status(200).json(await savePalettes(user,payload.rows||[]));
